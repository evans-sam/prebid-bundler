...
async function createTestFixture() {
    const rootDir = await Bun.file(join(tmpdir(), `prebid-test-${Date.now()}`)).name
        .replace(/[^/]+$/, "") + `prebid-test-${crypto.randomUUID()}`;
...
describe("getModulesForVersion", () => {
    ...
    test("throws error for non-existent version", async () => {
        await expect(getModulesForVersion(fixture.prebidDir, "99.99.99"))
...
describe("buildBundle", () => {
    ...
    test("spawns correct gulp command", async () => {
        const spawnCalls: Array<{cmd: string[]; opts: any}> = [];
    ...
    test("throws error for non-existent version", async () => {
        ...
        await expect(buildBundle(config, "99.99.99", ["test"]))
            .rejects.toThrow("Version 99.99.99 not found");
    });
    ...
    test("handles build failure with non-zero exit code", async () => {
        ...
        await expect(buildBundle(config, "10.20.0", ["test"]))
    ...
    test("handles build timeout", async () => {
        ...
        await expect(buildBundle(config, "10.20.0", ["test"]))
            .rejects.toThrow("Build timed out after 100ms");
